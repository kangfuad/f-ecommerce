 ...
 * 
 * Automatically generates static HTML files for every product in public/data/products.json
...
const productsJsonPath = path.resolve(rootDir, 'public/data/products.json')
...
function runPrerender() {
  ...
  if (!fs.existsSync(productsJsonPath)) {
    console.warn('[Prerender-OG] public/data/products.json not found.')
    return
  }
  ...
  for (const product of productList) {
    ...
    // 2. Output directory: dist/produk/{slug}/index.html (if name slug is different)
  ...
  console.log(`[Prerender-OG] Successfully generated ${count} product Open Graph HTML pages for social media crawlers.`)
}
